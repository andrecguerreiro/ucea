...
class CachedModel:
    def __init__(self):
        # Resolve weights relative to this script so runs from other CWDs still work.
        # Optional override: set FIXEDBOX_MODEL_PATH to an explicit .pt file.
        model_path = os.environ.get("FIXEDBOX_MODEL_PATH", "").strip()
        if model_path:
            weights = Path(model_path).expanduser()
        else:
            weights = Path(__file__).resolve().parent / "best.pt"
...
def use_this_function(jsonbox,building_threshold,overlap_threshold):
    ...
    print(f"✅ Done computing IOU matrix!")
    matched_data = []
    ...
    for i, osm_feat in enumerate(geojson['features']):
        ...
        if iou_mat[i, best_match_idx] > 0.3: # the buildings must have a 30% overlap to be considered the SAME building
    ...
    print("Done!")
    print("🎉  All done! Map is live →"), map_html

#jsonbox = "{\"north\":38.71287282568031,\"south\":38.71177693488299,\"east\":-9.141268730163576,\"west\":-9.142599105834963}"

jsonbox = "{\"north\":38.75908758631297,\"south\":38.754436161483895,\"east\":-9.09365790365263,\"west\":-9.099595767680661}"

#jsonbox = "{\"north\":41.15128397131025,\"south\":41.141337691126424,\"east\":-8.597917556762697,\"west\":-8.610491752624513}"
building_threshold = 0.15
overlap_threshold = 0.25
use_this_function(jsonbox,building_threshold,overlap_threshold)
